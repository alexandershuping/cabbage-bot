...
class Expression:
	...
	def compile(self, stringExpression):
		...
		self.op = None
		tmpExpression = []
		pieces = _breakOver(stringExpression, _keywords)
...
class VarExpression:
	...
	def compile(self, stringExpression):
		''' Note: This function discards anything passed except the first
		    variable.
		'''
		vloc = _findIgnoreEscaped(stringExpression, '$')
		...
		eloc = _findFirstNotOf(vex,'abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ')[0]
		if eloc != -1:
			vex = vex[:eloc]
...
def _breakOver(source, keywordList):
	''' Breaks a single string (source) into a list of strings, separated
	    by the keywords in keywordList.

			Example: with the default _keywords and s='(4+5er342)*6',
			_breakOver(s, _keywords) returns 
			['(','4','+','5er342',')','*','6']
	'''
	pieces = []
	dex = 0
	while True:
		inst = _findFirstOf(source, keywordList, dex)
		pos = inst[0]
		kword = inst[1]
		if pos == -1:
			piece = source[dex:]
...
def _findFirstOf(source, keywordList, dex=0):
	''' Finds the first instance of any of the keywords in keywordList in
	    source, excepting escaped keywords (that is, keywords preceded by
			_escapeChar, default value \), beginning at dex
	...
	for keyword in keywordList:
		pos = _findIgnoreEscaped(source, keyword, dex)
		if pos != -1:
...
def _findFirstNotOf(source, allowed, dex=0):
...
def _findIgnoreEscaped(source, expr, dex=0, escapeChar=None):
	''' Finds the first instance of the sequence expr in source, starting
	    at dex, excepting sequences that begin with escapeChar (default is
			...
			backslash, i.e. \\) and x = 't', then	_findIgnoreEscaped(s,e) 
